#' @description An adjusted version of NbClust::NbClust with verbose parameter.
...
NbClust2 <- function(
  ...
  Indices.WBT <- function(x,cl,P,s,vv)
  {
    ...
    else {if(verbose) cat("Error: division by zero!")}
  ...
  if (any(indice == 27) || (indice == 31) || (indice == 32))
  {
    ...
    if(verbose) cat(paste ("*** : The Hubert index is a graphical method of determining the number of clusters.
  ...
  if (any(indice == 29) || (indice == 31) || (indice == 32))
  {
    ...
    if(verbose) cat(paste ("*** : The D index is a graphical method of determining the number of clusters.
  ...
  #  {
  #     if(verbose) cat(paste(numberObsAfter,"observations were used out of", numberObsBefore ,"possible observations due to missing values."))
  #  }

  #  if (numberObsAfter == numberObsBefore)
  #  {
  #     if(verbose) cat(paste("All", numberObsAfter,"observations were used.", "\n", "\n"))
  #  }
  ...
  {
    if(verbose) cat("*******************************************************************", "\n")
    if(verbose) cat("* Among all indices:                                               ", "\n")
    ...
    for(i in min.nc:max.nc)
    {
      ...
      if(length(vect)>0)
        if(verbose) cat("*",length(vect), "proposed", i,"as the best number of clusters", "\n")
    ...
    if(verbose) cat("\n","                  ***** Conclusion *****                           ", "\n", "\n")
    if(verbose) cat("* According to the majority rule, the best number of clusters is ",j , "\n", "\n", "\n")
    if(verbose) cat("*******************************************************************", "\n")
