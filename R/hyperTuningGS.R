...
hyperTuningGS <- function(
  ...
  for(i in 1:nrow(para.grid)){ # i=6
    ...
    if(!file.exists(path_fitness_train)){
      ...
      res.multi_auc <- multiclass.roc(Ys_train, Ys_train_pred, quiet = TRUE, direction = "auto")
      res.binary <- list();
      response_subtype <- as.character(unique(Ys_valid))
      for(i in 1:length(response_subtype)){ # i=1
        subtype.i <-  response_subtype[i]
        binary_i <- as.numeric(Ys_train %in% subtype.i)
        res.binary.i <- roc(binary_i, Ys_train_pred, quiet = TRUE)
        res.binary[[as.character(subtype.i)]] <- as.numeric(res.binary.i$auc)
      }
      names(res.binary) <- paste('binary_auc_',names(res.binary), sep = '')
      
      res.kappa <- kappa2(data.frame(Ys_train, Ys_train_pred), "unweighted")
      res.all <- cbind(
        multi_auc = as.numeric(res.multi_auc$auc),
        accuracy = mean(Ys_train_pred == Ys_train),
    ...
    if(!file.exists(path_fitness_valid)){
      ...
      res.multi_auc <- multiclass.roc(Ys_valid, Ys_valid_pred, quiet = TRUE, direction = "auto")
      res.binary <- list();
      response_subtype <- as.character(unique(Ys_valid))
      for(i in 1:length(response_subtype)){ # i=1
        subtype.i <-  response_subtype[i]
        binary_i <- as.numeric(Ys_valid %in% subtype.i)
        res.binary.i <- roc(binary_i, Ys_valid_pred, quiet = TRUE)
        res.binary[[as.character(subtype.i)]] <- as.numeric(res.binary.i$auc)
      }
      names(res.binary) <- paste('binary_auc_',names(res.binary), sep = '')
      
      res.kappa <- kappa2(data.frame(Ys_valid, Ys_valid_pred), "unweighted")
      res.all <- cbind(
        multi_auc = as.numeric(res.multi_auc$auc),
        accuracy = mean(Ys_valid_pred == Ys_valid),
        kappa = res.kappa$value,
        as.data.frame(res.binary)
      )
