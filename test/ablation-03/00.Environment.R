...
# Resolve paths from either the ablation-03 project directory (the normal
# RStudio/.Rproj working directory) or the repository root.  Keeping this in
# one place makes the scripts equally convenient to run manually and by AI.
.ablation03_resolve_dir <- function() {
  candidates <- character()
  args <- commandArgs(trailingOnly = FALSE)
  file_arg <- grep("^--file=", args, value = TRUE)
  if (length(file_arg) > 0L) {
    candidates <- c(candidates, dirname(sub("^--file=", "", file_arg[1L])))
  }
  candidates <- c(candidates, getwd())
  for (candidate in unique(candidates)) {
    candidate <- normalizePath(candidate, winslash = "/", mustWork = FALSE)
    if (file.exists(file.path(candidate, "ablation-03.Rproj"))) {
      return(candidate)
    }
  ...
  stop(
    "ablation-03: cannot locate ablation-03.Rproj from the current script or working directory.",
    call. = FALSE
  )
}

.ablation03_dir <- .ablation03_resolve_dir()
.ablation03_root <- normalizePath(
  file.path(.ablation03_dir, "..", ".."),
  winslash = "/",
  mustWork = TRUE
)
...
.ablation03_cache_root <- normalizePath(
...
# --- CCS d1 vs Direct-GSClassifier ablation ---
luckyBase::Plus.library(c(
  "CCS",
  "GSClassifier",
  "digest",
  "xgboost",
